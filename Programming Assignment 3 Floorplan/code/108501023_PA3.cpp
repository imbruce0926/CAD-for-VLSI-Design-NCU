// PA3_Floorplanning
// Input file: module counts, area of each module
// Output file: chip aspect and area, aspect of each module
// This program use Wong-Liu Algorithm and Stockmeyer Algorithm
// 2023/5/12 Version 14
...
int main(int argc, char *argv[]) {
    ...
    while((double(clock()-start)/CLOCKS_PER_SEC) < 7080) 
        spe = F.SA(spe);
...
vector<string> Floorplanning::init() {
    ...
    // set every thing, escepically pts

    Node *tmp = NULL;
    vector<string> spe;

    if(moduleCount <= 500 && moduleCount > 1) {
        pts = 64500;
        if(moduleCount == 10)
            srand(5);
    ...
    else if(moduleCount <= 10000 && moduleCount > 500) {
        pts = 20000;
        srand(time(NULL));
    }
    else if(moduleCount <= 15000 && moduleCount > 10000) {
        pts = 10000;
        srand(time(NULL));
    }
    else if(moduleCount <= 20000 && moduleCount > 15000) {
        pts = 8000;
        srand(time(NULL));
    }
    else if(moduleCount <= 55000 && moduleCount > 20000) {
        pts = 3000;
        srand(time(NULL));
    }
    ...
    if(moduleCount <= 10 && moduleCount > 1) {
        pts = 50000; // 10000 -- 15s (57323.17) | 15000 -- 15s (57323.17) (5.3/3)| 20000 -- 20s (57323.19) (3.9/2) | 30000 -- 30s (57323.09) (2.81/1) | 50000 -- 50s (57323.05) (1.7/1) | 70000 -- 7-80s (57323.05) (1.2/1)
        iter_T = 8;
    }
    else if(moduleCount <= 20 && moduleCount > 10) {
        pts = 21500; // 5000 -- 21s (89445.85) | 8000 -- 25s (89733.37) (3.1/2) | 10000 -- 33s (89698.06) (2.5/1) | 20000 -- 70s (89584.44) (1.2/1) | 22000 -- 7-80s (89572.93) (0.9/1)
        iter_T = 8;
    }
    else if(moduleCount <= 100 && moduleCount > 20) {
        pts = 390; // 400 -- 25s
        iter_T = 8;
    }
    else if(moduleCount <= 200 && moduleCount > 100) {
        pts = 150; // 150 -- 23s
        iter_T = 6; 
    }
    else if(moduleCount <= 300 && moduleCount > 200) {
        pts = 80; // 80 -- 30s
        iter_T = 6; 
    }
    else if(moduleCount <= 400 && moduleCount > 300) {
        pts = 50; // 50 -- 30s
        iter_T = 5.9; 
    }
    else if(moduleCount <= 500 && moduleCount > 400) {
        pts = 41; // 40 -- s
        iter_T = 6;
    }
    else if(moduleCount <= 1000 && moduleCount > 500) {
        pts = 25; // 25 -- 80s () (?/1)
        iter_T = 5;
    }
    else if(moduleCount <= 2000 && moduleCount > 1000) {
        pts = 4; // | 4 -- 90s () (?/1)
        iter_T = 4.9;
    }
    else if(moduleCount <= 3000 && moduleCount > 2000){
        pts = 3;
        iter_T = 4;
    }
    else if(moduleCount <= 4000 && moduleCount > 3000){
        pts = 2;
        iter_T = 3.5;
    }
    else {
...
double Floorplanning::init_temp() {
    ...
    // implement the approach that find T0 in Wong-Liu Algorithm
...
void Floorplanning::bottom_up(Node *ptr) {
    ...
    // implement Stockmeyer Algorithm
...
bool Floorplanning::legal_check(int j) {
    ...
    // called by M3, check for balloting property and normalized polish expression
...
void Floorplanning::find_node_ij(Node **iptr, Node **jptr, int i, int j) {
    ...
    // called by M1, find the operands in the tree
...
void Floorplanning::find_node_i(Node **iptr, int i, int j) {
    ...
    // called by M2, find the deepest 'V' or 'H' in the  tree
...
void Floorplanning::find_int_VH(int x, int *i, int *j) {
    ...
    // called by M3, if i(j) is int, then j(i) is 'V' or 'H'
...
int Floorplanning::find_int(int x) {
    ...
    // called by M1, randomly choose index of a interger
...
int Floorplanning::find_VH(int x) {
    ...
    // called by M2, randomly choose index of 'V' or 'H'
...
Node * Floorplanning::SPE_to_tree(vector<string> SPE) {
    ...
    // turn polish expression into tree
