...
vector<string> Floorplanning::init() {
    ...
    // if(moduleCount <= 10)
    //     pts = 300; // pancake not greater, no need to large possible for starter
    // else if(moduleCount <= 20)
    //     pts = 150; // pancake not greater, no need to large possible for starter
    ...
    //     pts = 7; // 1500pts 263087(6min) - 800pts 263143
    // else {
    // }
    ...
    time_t test = clock();
    bottom_up(root);
    cout <<fixed<<setprecision(10)<< double(clock()-start)/CLOCKS_PER_SEC << " s" << endl;
    top_down(root);
...
void Floorplanning::bottom_up(Node *ptr) {
    ...
    if((ptr->data == "V" || ptr->data == "H") && ptr->update == false) {
        ...
        if(ptr->data == "V") {
            ...
            sort(ptr->left->blks.begin(), ptr->left->blks.begin()+ptr->left->blks.size(), mycompare3);
            sort(ptr->right->blks.begin(), ptr->right->blks.begin()+ptr->right->blks.size(), mycompare3);
...
bool mycompare3(Block a, Block b){
   return a.aspect.height< b.aspect.height;
}
