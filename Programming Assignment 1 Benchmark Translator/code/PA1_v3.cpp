...
using namespace std;
    ...
    private:
        string line_, func_, name_, input_[MAX_SIZE], output_[MAX_SIZE];
        int inputCount_, outputCount_, gateCount_, typeCount_[8];
        Gate_info gates_[MAX_SIZE];

        void delete_blank();
        void func_judgement();
        void process_txt();
        void get_IO(string *, int *);
        void get_name();
        void comma_or_semicolon(string *, int *, string);
        void get_port();
...
void Benchmark_translator::get_IO(string *inout, int *inoutCount) {
...
void Benchmark_translator::comma_or_semicolon(string *IOput, int *cnt, string item) {

    // input string may end with "," or ";"
    // need to be judge in different way

    IOput[(*cnt)++] = line_.substr(line_.find("N") + 1, line_.find(item) - line_.find("N") - 1);
...
void Benchmark_translator::get_port() {
    ...
    while(line_.find("N") != string::npos) {
        ...
        gates_[gateCount_].port[gates_[gateCount_].port_cnt++] = line_.substr(line_.find("N") + 1, line_.find(",") - line_.find("N") - 1);
